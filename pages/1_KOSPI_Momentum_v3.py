...
# ─────────────────────────────────────────────────────────
st.set_page_config(
    page_title="KOSPI 200 모멘텀 전략 v3",
    page_icon="🇰🇷",
    layout="wide"
)
...
v3는 초기 리밸런싱에서 미래 유니버스를 쓰지 않도록 수정하고, 현재 추천 종목도 11-1 모멘텀 설정을 반영합니다.
""")
...
if run_btn:
    ...
    with tab2:
        ...
        p_ref_now = df_price[valid_latest].loc[ref_now]
        p_curr_now = df_price[valid_latest].loc[latest_date]
        p_past_now = df_price[valid_latest].loc[past_now]
        curr_mom   = ((p_ref_now - p_past_now) / p_past_now).replace([np.inf, -np.inf], np.nan).dropna()
        ...
        st.caption(
            ...
            f"모멘텀 기준 가격일: {ref_now.strftime('%Y-%m-%d')}"
        )
    ...
    st.download_button(
        label="💾 결과 엑셀 다운로드",
        data=buf.getvalue(),
        file_name=f"momentum_v3_{pd.Timestamp.today().strftime('%Y%m%d')}.xlsx",
