...
def line_chart(data: pd.DataFrame, title: str, yaxis: str = "", percent: bool = False, mdd: dict | None = None):
    fig = go.Figure()
    palette = [COLORS["strategy"], COLORS["kodex_semiconductor"], COLORS["benchmark"], COLORS["kodex_bond"]]
...
with tab_execution:
    ...
    st.dataframe(execution.style.format({"Latest Price": "₩{:,.0f}", "Target Weight": "{:.1%}",
...
with tab_comparison:
    ...
    fig = go.Figure()
    for column, color in zip(yearly.columns, [COLORS["strategy"], COLORS["kodex_semiconductor"], COLORS["benchmark"], COLORS["kodex_bond"]]):
...
with tab_monthly:
    ...
    d1.download_button("Download daily backtest CSV", daily_export.to_csv(index_label="date").encode("utf-8-sig"),
                       file_name="kodex_semiconductor_vol_target_daily.csv", mime="text/csv", use_container_width=True)
    d2.download_button("Download summary CSV", summary.to_csv(index=False).encode("utf-8-sig"),
                       file_name="kodex_semiconductor_vol_target_summary.csv", mime="text/csv", use_container_width=True)
