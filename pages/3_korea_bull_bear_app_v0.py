"""KODEX 200 / KODEX Leverage ON/OFF strategy v0.

v0 keeps the previous v5 core idea because it fits the KODEX data better:
...
st.set_page_config(page_title="KODEX ON/OFF v0", page_icon="KR", layout="wide")
st.title("KODEX 200 / Leverage ON-OFF Strategy v0")
st.caption(
    "Core v3 logic retained: KODEX 200 trend + realized-volatility filter. "
    "v0 is the prior baseline with regime diagnostics, recent-period checks, and parameter sensitivity."
)
...
if not run_btn:
    st.info("Adjust the settings, then run the backtest. v0 keeps the prior ON/OFF core and adds diagnostics.")
    st.stop()
...
with tab_trades:
    if trade_log.empty:
        ...
        st.dataframe(shown, use_container_width=True, hide_index=True)
        st.download_button("Trade Log CSV", trade_log.to_csv(index=False).encode("utf-8-sig"), "kodex_onoff_v0_trades.csv", "text/csv")
...
with tab_monthly:
    ...
    st.download_button("Monthly Returns CSV", monthly.reset_index().rename(columns={"index": "Date"}).to_csv(index=False).encode("utf-8-sig"), "kodex_onoff_v0_monthly.csv", "text/csv")
