"""KODEX 200 / leverage bull-bear backtest.

Signals
- KODEX 200 close versus its moving average.
...
st.set_page_config(page_title="KODEX Bull/Bear Backtest", page_icon="KR", layout="wide")
st.title("KODEX 200 Bull / Bull Mix / Bear Backtest")
st.caption("KODEX 200 추세 + 전일 TNX 금리 필터 + 종가/익일 시가 체결 선택")
...
| Bull Mix | KODEX 200 > MA{kodex_ma_window} and 전일 TNX > 전일 TNX MA{tnx_ma_window} | KODEX 200 {mix_kodex}% + 레버리지 {mix_lev}% |
| Bull Full | KODEX 200 > MA{kodex_ma_window} and 전일 TNX <= 전일 TNX MA{tnx_ma_window} | KODEX 200 {full_kodex}% + 레버리지 {full_lev}% |
"""
    )
...
with tab_signals:
    ...
    st.line_chart(pd.DataFrame({"KODEX 200": kodex_200["close"].reindex(common_idx).ffill(), f"MA{kodex_ma_window}": kodex_ma_full.reindex(common_idx).ffill()}), height=280)
...
with tab_monthly:
    ...
    monthly = pd.DataFrame({"전략": monthly_strategy, "KODEX 200": monthly_benchmark}).dropna()
