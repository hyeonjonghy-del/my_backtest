"""
한국판 Bull/Bull Mix/Bear 전략 - 코스닥150 버전
────────────────────────────────────────
...
상태 판단:
  ...
  Bull Mix : KOSDAQ150 > MA150 & TNX > TNX MA120
  Bull Full: KOSDAQ150 > MA150 & TNX ≤ TNX MA120

포트폴리오:
  Bull Full → KODEX 코스닥150 (229200) 100%
  Bull Mix  → KODEX 코스닥150레버리지 (233740) N% + KODEX 코스닥150 (100-N)%
...
with st.sidebar:
    ...
    lev_ratio = st.slider("Bull Mix 시 레버리지 비중 (%)", 10, 70, 50, 5)
    st.caption(f"Bull Mix = 코스닥레버리지 {lev_ratio}% + 코스닥150 {100-lev_ratio}%")
...
| 상태 | 조건 | 포트폴리오 |
|------|------|-----------|
| 🐂 Bull Full | KOSDAQ150 > MA150 & TNX ≤ MA120 | KODEX 코스닥150 100% |
| ⚠️ Bull Mix | KOSDAQ150 > MA150 & TNX > MA120 | 코스닥150레버리지 50% + 코스닥150 50% |
| 🐻 Bear | KOSDAQ150 < MA150 | KODEX 단기채권 100% |
...
def load_kosdaq150(start_str: str, end_str: str) -> pd.Series:
    """KOSDAQ150 지수. 실패 시 KODEX 코스닥150(229200) 대체."""
    try:
...
if run_btn:
    ...
    kd150_ma = kosdaq150.rolling(ma_trend).mean()
    ...
    kd       = kosdaq150.reindex(common_idx).ffill()
    kd_ma    = kd150_ma.reindex(common_idx).ffill()
    ...
    label_map = {
        "Bull_Full": "Bull Full — KODEX 코스닥150 100%",
        "Bull_Mix":  f"Bull Mix — 코스닥150레버리지 {lev_ratio}% + 코스닥150 {100-lev_ratio}%",
