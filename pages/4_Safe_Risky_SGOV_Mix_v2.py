...
st.title("🛡️ Safe/Risky/Cash Mix Strategy v3 (Enhanced)")
st.markdown("""
**v3 주요 개선 사항:**
- 🔧 **Whipsaw 필터:** N일 연속 조건 충족 시에만 포지션 전환 (횡보장 오신호 방지)
- ⚖️ **변동성 기반 포지션 사이징:** 시장 변동성에 따라 UPRO 비중 자동 조절 (MDD 감소)
- 🌏 **보조 시장 필터:** QQQ 추가 확인으로 신호 정확도 향상 (선택)
- 📊 **성과 지표 보강:** Sharpe, Calmar, 승률, 평균 보유기간 추가
""")
st.markdown("---")
...
with st.sidebar:
    ...
    use_whipsaw     = st.checkbox("Whipsaw 필터 사용", value=True)
    confirm_days    = st.number_input("신호 확정 기간 (일)", value=3, min_value=1, max_value=10,
                                       help="이평선 위/아래에 N일 연속 유지 시에만 포지션 전환")
    if use_whipsaw:
