"""KODEX KOSDAQ150 / KODEX KOSDAQ150 Leverage ON/OFF strategy v3.

This strategy uses only the two KODEX KOSDAQ150 daily OHLCV series:
- Signal asset: KODEX KOSDAQ150.
- Trading asset: KODEX KOSDAQ150 Leverage.
- Hold KODEX KOSDAQ150 Leverage only when trend and volatility filters pass.
- Hold cash otherwise.
...
st.set_page_config(page_title="KOSDAQ150 ON/OFF v3", page_icon="KR", layout="wide")
st.title("KODEX KOSDAQ150 / Leverage ON-OFF Strategy v3")
st.caption(
    "KOSDAQ150 daily-price strategy: trend + realized-volatility filter. "
    "Designed for aggressive return seeking with after-close execution modeling."
)
...
def render_yearly_bars(strategy_nav: pd.Series, kodex_nav: pd.Series, lev_nav: pd.Series) -> None:
    yearly = pd.DataFrame(
        {
            ...
            "KODEX KOSDAQ150": (1 + kodex_nav.pct_change().fillna(0)).groupby(kodex_nav.index.year).prod() - 1,
    ...
    width = 0.25
    bars = [("Strategy", COLORS["strategy"], -width), ("KODEX KOSDAQ150", COLORS["kodex200"], 0), ("KODEX Leverage", COLORS["leverage"], width)]
...
with st.sidebar:
    ...
    ma_window = st.slider("KODEX KOSDAQ150 MA", 20, 250, 80, 5)
    vol_window = st.slider("Realized volatility window", 5, 120, 20, 1)
    vol_threshold_pct = st.slider("Realized volatility cap (%)", 10, 150, 80, 5)
...
| Entry / hold | KODEX KOSDAQ150 close > MA{ma_window} AND {vol_source} RV{vol_window} < {vol_threshold_pct}% |
...
if not run_btn:
    st.info("Adjust the settings, then run the backtest. Defaults are aggressive and return-focused for KOSDAQ150.")
    st.stop()
...
vol_price = kodex_close if vol_source == "KODEX KOSDAQ150" else lev_close
...
st.caption(
    ...
    f"KODEX KOSDAQ150 {latest_close:,.0f} / MA{ma_window} {latest_ma:,.0f} / "
    f"{vol_source} RV{vol_window} {latest_vol:.1%} / cap {vol_threshold:.0%}"
)

cols = st.columns(6)
cols[0].metric("Total Return", f"{strategy_metrics['total']:.1%}", f"KOSDAQ150 {benchmark_200_metrics['total']:.1%}")
    ...
    nav_chart = pd.DataFrame(
        {
            ...
            "KODEX KOSDAQ150 B&H": benchmark_200 / benchmark_200.iloc[0],
    ...
    dd_chart = pd.DataFrame(
        {
            ...
            "KODEX KOSDAQ150 DD": benchmark_200_metrics["dd"],
    ...
    for period_name, period_start in [("Full", None), (f"Recent {recent_years}Y", recent_start), ("Since 2025", post_2025)]:
        table = metrics_table(
            [
                ...
                ("KODEX KOSDAQ150", slice_nav(benchmark_200, period_start)),
...
with tab_sensitivity:
    if not run_sensitivity:
        ...
        ma_values = sorted(set([60, 80, 100, 120, ma_window]))
        vol_values = sorted(set([10, 20, 30, vol_window]))
        threshold_values = sorted(set([60, 70, 80, 90, vol_threshold_pct]))
        records = []
...
with tab_signal:
    trend_chart = pd.DataFrame({"KODEX KOSDAQ150": kodex_close.reindex(common_idx), f"MA{ma_window}": ma.reindex(common_idx)})
    ...
    recent_signal = pd.DataFrame(
        {
            ...
            "KODEX KOSDAQ150": kodex_close.reindex(common_idx),
...
with tab_trades:
    if trade_log.empty:
        ...
        st.dataframe(shown, use_container_width=True, hide_index=True)
        st.download_button("Trade Log CSV", trade_log.to_csv(index=False).encode("utf-8-sig"), "kosdaq150_onoff_v3_trades.csv", "text/csv")
...
with tab_monthly:
    ...
    monthly = pd.DataFrame({"Strategy": monthly_strategy, "KODEX KOSDAQ150": monthly_200, "KODEX Leverage": monthly_lev}).dropna()
    ...
    st.download_button("Monthly Returns CSV", monthly.reset_index().rename(columns={"index": "Date"}).to_csv(index=False).encode("utf-8-sig"), "kosdaq150_onoff_v3_monthly.csv", "text/csv")
