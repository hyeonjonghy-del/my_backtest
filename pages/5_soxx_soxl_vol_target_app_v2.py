"""SOXX / SOXL trend and volatility-target backtest v2."""
...
st.set_page_config(page_title="SOXX/SOXL Vol Target Backtest V2", page_icon="US", layout="wide")
st.title("SOXX / SOXL Volatility Target Backtest V2")
st.caption(
    "Default: SOXX MA30 > MA200, target volatility 45%, SOXL cap 50%, "
    "max SOXX-equivalent risk exposure 1.5x, 20% bull risk share to SOXX, "
    "20% SOXX in bear regimes"
)
...
def build_strategy_weights(
    ...
    elif allocation_mode == "Risk-balanced SOXX blend":
        soxx_w = (desired_risk * soxx_risk_share).clip(0, 1)
        soxl_w = ((desired_risk - soxx_w) / 3).clip(0, soxl_cap)
        risk_used = soxx_w + soxl_w * 3
        soxx_w = (soxx_w + (desired_risk - risk_used).clip(lower=0)).clip(0, 1 - soxl_w)
...
def calc_target_weight(
    ...
    if allocation_mode == "Risk-adjusted":
        ...
        soxl_w = min(max((desired_risk - soxx_w) / 3, 0.0), soxl_cap)
        risk_used = soxx_w + soxl_w * 3
...
with st.sidebar:
    ...
    allocation_mode = st.selectbox(
        "Allocation mode",
        ["Risk-balanced SOXX blend", "Risk-adjusted", "Capital-first", "Fixed bull weights"],
        index=0,
    )
    soxx_risk_share = st.slider("SOXX risk share in bull regime (%)", 0, 60, 20, 5) / 100
...
| SOXX risk share | {soxx_risk_share:.0%} of bull-regime risk budget |
