"""SOXX / SOXL trend and volatility-target backtest v3."""
...
st.set_page_config(page_title="SOXX/SOXL Vol Target Backtest V3", page_icon="US", layout="wide")
st.title("SOXX / SOXL Volatility Target Backtest V3")
st.caption(
    "Default: Strong Bull uses SOXL tactically, Weak Bull shifts toward SOXX, "
    "Bear keeps mostly cash with a small SOXX sleeve"
)
...
latest_regime = str(regime_signal.reindex(weights.index).ffill().iloc[-1])
    ...
    recent = pd.DataFrame(
        {
            ...
            "Regime": regime_signal.reindex(common_idx),
