"""SOXX / SOXL trend and volatility-target backtest v6."""
...
st.set_page_config(page_title="SOXX/SOXL Vol Target Backtest V6", page_icon="US", layout="wide")
st.title("SOXX / SOXL Volatility Target Backtest V6")
st.caption(
    "Default: Strong Bull keeps the v5 volatility target, while high-volatility bull regimes split "
    "between upside-led participation and downside-stress defense."
)
...
def split_high_vol_bull_regime(
    price: pd.Series,
    fast_ma: pd.Series,
    ...
    recent_return = (price / price.shift(recent_return_window) - 1).rename("Recent Return")
    recent_high = price.rolling(drawdown_window).max()
    pullback = (price / recent_high - 1).rename("Pullback From Recent High")
    short_trend_ok = ((price > fast_ma) & (fast_ma > fast_ma.shift(5))).rename("Short Trend OK")

    if lag_for_execution:
        upside_vol_signal = upside_vol.shift(1)
        downside_vol_signal = downside_vol.shift(1)
        recent_return_signal = recent_return.shift(1)
        pullback_signal = pullback.shift(1)
        short_trend_ok_signal = short_trend_ok.shift(1).fillna(False)
    else:
        upside_vol_signal = upside_vol
        downside_vol_signal = downside_vol
        recent_return_signal = recent_return
        pullback_signal = pullback
        short_trend_ok_signal = short_trend_ok.fillna(False)

    downside_stress = (
        (recent_return_signal < 0)
        | (pullback_signal <= -pullback_threshold)
        | (downside_vol_signal > upside_vol_signal * downside_vol_ratio)
        | (~short_trend_ok_signal)
    ).fillna(False).rename("Downside Stress Signal")

    regime = base_regime.copy()
    weak_bull = base_regime == "Weak Bull"
    regime.loc[weak_bull & downside_stress] = "Downside Stress Bull"
    regime.loc[weak_bull & ~downside_stress] = "Upside High-Vol Bull"
    diagnostics = {
...
def build_strategy_weights(
    ...
    weak_soxl_cap: float,
    upside_high_vol_soxx_weight: float,
    upside_high_vol_soxl_weight: float,
    stress_high_vol_soxx_weight: float,
    stress_high_vol_soxl_weight: float,
    ...
    weights["SOXL"] = np.select(
        ...
        [strong_soxl, weak_soxl, upside_high_vol_soxl_weight, stress_high_vol_soxl_weight],
        default=0.0,
    )
    weights["SOXX"] = np.select(
        ...
        [strong_soxx, weak_soxx, upside_high_vol_soxx_weight, stress_high_vol_soxx_weight],
        default=bear_soxx,
    )
...
def calc_target_weight(
    ...
    if regime == "Upside High-Vol Bull":
        target = pd.Series({"SOXX": upside_high_vol_soxx_weight, "SOXL": upside_high_vol_soxl_weight}).clip(0, 1)
        return target / target.sum() if target.sum() > 1 else target

    if regime == "Downside Stress Bull":
        target = pd.Series({"SOXX": stress_high_vol_soxx_weight, "SOXL": stress_high_vol_soxl_weight}).clip(0, 1)
        return target / target.sum() if target.sum() > 1 else target
...
with st.sidebar:
    ...
    pullback_threshold = st.slider("Stress pullback threshold (%)", 1, 20, 5, 1) / 100
    downside_vol_ratio = st.slider("Downside stress vol ratio", 0.5, 2.0, 1.0, 0.1)
    upside_high_vol_soxx_weight = st.slider("Upside high-vol SOXX weight (%)", 0, 100, 60, 5) / 100
    upside_high_vol_soxl_weight = st.slider("Upside high-vol SOXL weight (%)", 0, 100, 25, 5) / 100
    stress_high_vol_soxx_weight = st.slider("Stress high-vol SOXX weight (%)", 0, 100, 50, 5) / 100
    stress_high_vol_soxl_weight = st.slider("Stress high-vol SOXL weight (%)", 0, 100, 10, 5) / 100
    if upside_high_vol_soxx_weight + upside_high_vol_soxl_weight > 1:
        st.info("Upside high-vol weights exceed 100%, so they will be normalized proportionally.")
    if stress_high_vol_soxx_weight + stress_high_vol_soxl_weight > 1:
...
| Weak Bull base allocation | {weak_risk_multiplier:.0%} risk budget, SOXX gets {weak_soxx_risk_share:.0%}, SOXL cap {weak_soxl_cap:.0%} |
| Upside High-Vol Bull | Weak Bull without downside-stress confirmation: SOXX {upside_high_vol_soxx_weight:.0%}, SOXL {upside_high_vol_soxl_weight:.0%}, Cash {max(0, 1 - upside_high_vol_soxx_weight - upside_high_vol_soxl_weight):.0%} |
| Downside Stress Bull | Weak Bull with recent return / pullback / downside-vol stress: SOXX {stress_high_vol_soxx_weight:.0%}, SOXL {stress_high_vol_soxl_weight:.0%}, Cash {max(0, 1 - stress_high_vol_soxx_weight - stress_high_vol_soxl_weight):.0%} |
| Downside-stress confirmation | Recent {recent_return_window}d return < 0%, or pullback from {drawdown_window}d high <= -{pullback_threshold:.0%}, or downside RV > upside RV x {downside_vol_ratio:.1f}, or short trend is not rising |
...
regime_signal, regime_diagnostics = split_high_vol_bull_regime(
    price,
    fast_ma,
    regime_signal,
    close_ret_soxx_full,
...
close_regime_signal, close_regime_diagnostics = split_high_vol_bull_regime(
    price,
    fast_ma,
    close_regime_signal,
    close_ret_soxx_full,
...
weights_full = build_strategy_weights(
    ...
    weak_soxl_cap,
    upside_high_vol_soxx_weight,
    upside_high_vol_soxl_weight,
    stress_high_vol_soxx_weight,
    stress_high_vol_soxl_weight,
    ...
    recent = pd.DataFrame(
        {
            ...
            "Upside RV": close_regime_diagnostics["upside_vol"].reindex(common_idx),
            "Downside RV": close_regime_diagnostics["downside_vol"].reindex(common_idx),
            f"{recent_return_window}d Return": close_regime_diagnostics["recent_return"].reindex(common_idx),
            f"{drawdown_window}d Pullback": close_regime_diagnostics["pullback"].reindex(common_idx),
            "Short Trend OK": close_regime_diagnostics["short_trend_ok"].reindex(common_idx),
            "Downside Stress": close_regime_diagnostics["downside_stress"].reindex(common_idx),
