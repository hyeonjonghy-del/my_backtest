"""KODEX KOSDAQ 150 / KODEX KOSDAQ 150 Leverage ON/OFF strategy v1.

v1 keeps the v0 core idea and adds a high-volatility bull fallback:
- Signal asset: KODEX KOSDAQ 150.
- Main trading asset: KODEX KOSDAQ 150 Leverage.
- Hold KODEX KOSDAQ 150 Leverage when trend and volatility filters pass.
- Optionally hold KODEX KOSDAQ 150 + cash when trend passes but RV is above the cap.
...
st.set_page_config(page_title="KOSDAQ 150 ON/OFF v1", page_icon="KR", layout="wide")
title_col, run_col = st.columns([4, 1])
with title_col:
    st.title("KODEX KOSDAQ 150 / Leverage ON-OFF Strategy v1")
with run_col:
    st.write("")
...
st.caption(
    ...
    "the strategy can hold KODEX KOSDAQ 150 plus cash instead of moving fully to cash."
)
...
def render_static_line(data: pd.DataFrame, title: str, ylabel: str = "", height: float = 3.8, percent_axis: bool = False) -> None:
    ...
    palette = [COLORS["strategy"], COLORS["kosdaq150"], COLORS["leverage"], COLORS["ma"], COLORS["vol"], COLORS["threshold"], COLORS["dd"]]
...
def render_yearly_bars(strategy_nav: pd.Series, kodex_nav: pd.Series, lev_nav: pd.Series) -> None:
    yearly = pd.DataFrame(
        {
            ...
            "KODEX KOSDAQ 150": (1 + kodex_nav.pct_change().fillna(0)).groupby(kodex_nav.index.year).prod() - 1,
            "KODEX KOSDAQ 150 Leverage": (1 + lev_nav.pct_change().fillna(0)).groupby(lev_nav.index.year).prod() - 1,
        }
    ).dropna(how="all") * 100
    ...
    width = 0.25
    bars = [("Strategy", COLORS["strategy"], -width), ("KODEX KOSDAQ 150", COLORS["kosdaq150"], 0), ("KODEX KOSDAQ 150 Leverage", COLORS["leverage"], width)]
...
def build_target_weights(
    ...
    return pd.DataFrame(
        {
            "KODEX KOSDAQ 150 Leverage": lev_weight.clip(0.0, 1.0),
            "KODEX KOSDAQ 150": kodex_weight.clip(0.0, 1.0),
...
def format_allocation(weights: pd.Series) -> str:
    ordered_assets = ["KODEX KOSDAQ 150 Leverage", "KODEX KOSDAQ 150", "Cash"]
    parts = []
...
def portfolio_trade_row(
    ...
    return {
        ...
        "Old KODEX KOSDAQ 150 Leverage": float(old_weights.get("KODEX KOSDAQ 150 Leverage", 0.0)),
        "Old KODEX KOSDAQ 150": float(old_weights.get("KODEX KOSDAQ 150", 0.0)),
        "Old Cash": float(old_weights.get("Cash", 0.0)),
        "New KODEX KOSDAQ 150 Leverage": float(new_weights.get("KODEX KOSDAQ 150 Leverage", 0.0)),
        "New KODEX KOSDAQ 150": float(new_weights.get("KODEX KOSDAQ 150", 0.0)),
...
def build_multi_asset_execution_plan(
    ...
    summary = {
        "effective_value": effective_value,
        "target_leverage_weight": float(target_weights.get("KODEX KOSDAQ 150 Leverage", 0.0)),
        "target_kodex_weight": float(target_weights.get("KODEX KOSDAQ 150", 0.0)),
...
with st.sidebar:
    ...
    ma_window = st.slider("KODEX KOSDAQ 150 MA", 20, 250, 100, 5)
    ...
    vol_source = st.selectbox("Volatility source", ["KODEX KOSDAQ 150", "KODEX KOSDAQ 150 Leverage"], index=0)
    use_high_vol_fallback = st.checkbox("Use RV cap fallback", value=True)
    high_vol_kodex_weight_pct = st.slider("KODEX KOSDAQ 150 weight when RV cap fails (%)", 0, 100, 50, 5)

    st.subheader("Position / Cost")
    leverage_weight_pct = st.slider("KODEX KOSDAQ 150 Leverage weight when signal passes (%)", 0, 100, 100, 5)
    ...
    account_state = render_domestic_account_controls(
        {
            "KODEX KOSDAQ 150": KODEX_KOSDAQ_150,
            "KODEX KOSDAQ 150 Leverage": KODEX_KOSDAQ_150_LEVERAGE,
        },
        "kosdaq150_bull_bear",
    ...
    current_lev_shares = float(account_state["shares"].get(KODEX_KOSDAQ_150_LEVERAGE, 0.0))
    current_kodex_shares = float(account_state["shares"].get(KODEX_KOSDAQ_150, 0.0))
...
|---|---|
| Signal asset | KODEX KOSDAQ 150 |
| Trading asset | KODEX KOSDAQ 150 Leverage |
| Entry / hold | KODEX KOSDAQ 150 close > MA{ma_window} AND {vol_source} RV{vol_window} < {vol_threshold_pct}% |
| High-vol bull fallback | {'On' if use_high_vol_fallback else 'Off'}; if trend passes but RV cap fails, hold KODEX KOSDAQ 150 {high_vol_kodex_weight_pct}%, cash {100 - high_vol_kodex_weight_pct}% |
...
| Main position | KODEX KOSDAQ 150 Leverage {leverage_weight_pct}%, cash {100 - leverage_weight_pct}% |
"""
    )
...
progress.progress(20, text="Loading KODEX KOSDAQ 150 data...")
kodex_200 = load_krx_ohlcv(KODEX_KOSDAQ_150, extended_start_str, end_str)
progress.progress(45, text="Loading KODEX KOSDAQ 150 Leverage data...")
kodex_lev = load_krx_ohlcv(KODEX_KOSDAQ_150_LEVERAGE, extended_start_str, end_str)
...
vol_price = kodex_close if vol_source == "KODEX KOSDAQ 150" else lev_close
...
ret_co = pd.DataFrame({"KODEX KOSDAQ 150 Leverage": ret_lev_co, "KODEX KOSDAQ 150": ret_kodex_co, "Cash": 0.0}, index=common_idx)
ret_oc = pd.DataFrame({"KODEX KOSDAQ 150 Leverage": ret_lev_oc, "KODEX KOSDAQ 150": ret_kodex_oc, "Cash": 0.0}, index=common_idx)
ret_cc = pd.DataFrame({"KODEX KOSDAQ 150 Leverage": ret_lev_cc, "KODEX KOSDAQ 150": ret_kodex_cc, "Cash": 0.0}, index=common_idx)
...
execution_plan, execution_summary = build_holdings_trade_plan(
    ...
    {
        "KODEX KOSDAQ 150 Leverage": float(kodex_lev["close"].reindex(common_idx).ffill().iloc[-1]),
        "KODEX KOSDAQ 150": float(kodex_200["close"].reindex(common_idx).ffill().iloc[-1]),
    },
    {
        "KODEX KOSDAQ 150 Leverage": current_lev_shares,
        "KODEX KOSDAQ 150": current_kodex_shares,
    },
    current_cash,
    account_value,
)
...
st.success(
    ...
    f"KODEX KOSDAQ 150 Leverage {latest_weights['KODEX KOSDAQ 150 Leverage']:.0%}, "
    f"KODEX KOSDAQ 150 {latest_weights['KODEX KOSDAQ 150']:.0%}, Cash {latest_weights['Cash']:.0%}"
)
st.caption(
    f"Latest leverage signal: {'Pass' if latest_signal else 'Wait'} | "
    f"Trend: {'Pass' if latest_trend_signal else 'Wait'} | "
    f"KODEX KOSDAQ 150 {latest_close:,.0f} / MA{ma_window} {latest_ma:,.0f} / "
    ...
    nav_chart = pd.DataFrame(
        {
            ...
            "KODEX KOSDAQ 150 B&H": benchmark_200 / benchmark_200.iloc[0],
            "KODEX KOSDAQ 150 Leverage B&H": benchmark_lev / benchmark_lev.iloc[0],
        }
    )
    ...
    dd_chart = pd.DataFrame(
        {
            ...
            "KODEX KOSDAQ 150 DD": benchmark_200_metrics["dd"],
            "KODEX KOSDAQ 150 Leverage DD": benchmark_lev_metrics["dd"],
        }
    ) * 100
    render_static_line(dd_chart, "Drawdown", "%", 3.0, True)

    weight_chart = weight_s[["KODEX KOSDAQ 150 Leverage", "KODEX KOSDAQ 150", "Cash"]].clip(0.0, 1.0)
...
with tab_execution:
    ...
    exec_cols[0].metric(
        "LEV Target",
        f"{float(target_weights_for_plan.get('KODEX KOSDAQ 150 Leverage', 0.0)):.0%}",
    )
    exec_cols[1].metric(
        "KODEX200 Target",
        f"{float(target_weights_for_plan.get('KODEX KOSDAQ 150', 0.0)):.0%}",
    )
    ...
    for period_name, period_start in [("Full", None), (f"Recent {recent_years}Y", recent_start), ("Since 2025", post_2025)]:
        table = metrics_table(
            [
                ...
                ("KODEX KOSDAQ 150", slice_nav(benchmark_200, period_start)),
                ("KODEX KOSDAQ 150 Leverage", slice_nav(benchmark_lev, period_start)),
            ]
        )
...
with tab_signal:
    trend_chart = pd.DataFrame({"KODEX KOSDAQ 150": kodex_close.reindex(common_idx), f"MA{ma_window}": ma.reindex(common_idx)})
    ...
    recent_signal = pd.DataFrame(
        {
            ...
            "KODEX KOSDAQ 150 Leverage Weight": weight_s["KODEX KOSDAQ 150 Leverage"],
            "KODEX KOSDAQ 150 Weight": weight_s["KODEX KOSDAQ 150"],
            "Cash Weight": weight_s["Cash"],
            "KODEX KOSDAQ 150": kodex_close.reindex(common_idx),
...
with tab_trades:
    if trade_log.empty:
        ...
        pct_cols = [
            "Old Weight",
            "New Weight",
            "Old KODEX KOSDAQ 150 Leverage",
            "Old KODEX KOSDAQ 150",
            "Old Cash",
            "New KODEX KOSDAQ 150 Leverage",
            "New KODEX KOSDAQ 150",
            "New Cash",
            "Turnover",
        ]
        for col in pct_cols:
        ...
        st.dataframe(shown, use_container_width=True, hide_index=True)
        st.download_button("Trade Log CSV", trade_log.to_csv(index=False).encode("utf-8-sig"), "kodex_kosdaq150_onoff_v1_trades.csv", "text/csv")
...
with tab_monthly:
    ...
    monthly = pd.DataFrame({"Strategy": monthly_strategy, "KODEX KOSDAQ 150": monthly_200, "KODEX KOSDAQ 150 Leverage": monthly_lev}).dropna()
